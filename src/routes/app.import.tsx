...
function guessMap(headers: string[]): Record<string, Field | ""> {
  ...
  for (const h of headers) {
    ...
    else if (n.includes("department") || n.includes("costcenter") || n.includes("dept"))
      match = "department";
    else if (n.includes("budget")) match = "budgeted_amount";
    else if (n.includes("actual") || n.includes("spend") || n.includes("spent"))
      match = "actual_amount";
    else if (
      n.includes("period") ||
      n.includes("quarter") ||
...
function ImportPage() {
  ...
  function downloadTemplate() {
    ...
    a.href = url;
    a.download = "budget-pulse-template.csv";
    a.click();
      ...
      <>
        ...
        <PageBody>
          <div className="rounded-lg border border-border p-6 text-sm text-muted-foreground">
            Your role is viewer, which is read-only. Ask an admin to import data or change your
            role.
          </div>
        </PageBody>
      </>
        ...
        {rows.length === 0 ? (
          <label className="grid cursor-pointer place-items-center rounded-lg border border-dashed border-border p-14 text-center hover:bg-accent/40">
            ...
            <div className="mt-1 text-sm text-muted-foreground">
              Or download the template first and fill it in.
            </div>
          ...
          <div className="space-y-6">
                ...
                {headers.map((h) => (
                  <div key={h} className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setMapping((m) => ({ ...m, [h]: v === "__skip" ? "" : (v as Field) }))
                      }
                    >
                      <SelectTrigger>
                  ...
                  <thead className="sticky top-0 bg-background">
                    <tr>
                      {headers.map((h) => (
                        <th
                          key={h}
                          className="border-b border-border p-2 text-left font-mono text-xs"
                        >
                          {h}
                        </th>
                      ))}
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              <Button onClick={commit} disabled={busy} className="w-full sm:w-auto min-h-11">
